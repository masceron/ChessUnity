...
namespace Third_Party.Autotiles3D.Scripts.Core.Editor
{
    ...
    public class Autotiles3D_BlockBehaviourInspector : UnityEditor.Editor
    {
        ...
        private bool CheckIfCanRandomize()
        {
            var node = _baseBlock.GetInternalNode();
            if (node == null)
                return false;

            return node.TryRandomize(out var random);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            var tile = _baseBlock.GetTile();
            Autotiles3D_TileGroup group = null;
            if (tile == null)
            {
                EditorGUILayout.LabelField("<color=red> LINK TO TILE IS MISSING </color>",
                    Autotiles3DUtility.RichStyle);
                EditorGUILayout.LabelField(
                    "This might be due to migration to Autotiles3D version 1.3 or because you have deleted TileGroups/Tiles");
                EditorGUILayout.LabelField(
                    "Don't panic, this is not critical. However you won't be able to use new features (such as \"View Tile\" or \"Randomize\") for this block until you reconnect to your tile");
                EditorGUILayout.LabelField(
                    "If you would like to reconnect, you can manually update the TileGroup's name and the tile's name for this block here once, and Autotiles3D automatically links things up again!");
            ...
            var internalNode = _baseBlock.GetInternalNode();
            ...
            {
                EditorGUILayout.LabelField(
                    "Internal node missing! This should not happen. Have you messed with the hierarchy?");
            }


            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            var isBaked = _baseBlock.IsBaked ? "(IS BAKED)" : "";

            var name = tile != null ? tile.Name : "Missing Name";
            ...
            {
                var node = _baseBlock.GetInternalNode();
                if (node != null) node.VerifyInstance();
            }
