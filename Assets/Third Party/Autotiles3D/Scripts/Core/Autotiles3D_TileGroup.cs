...
namespace Third_Party.Autotiles3D.Scripts.Core
{
    ...
    {
        //serialized data
        public List<Autotiles3D_Tile> Tiles = new();
        private Dictionary<int, Autotiles3D_Tile> _map = new();
        private Dictionary<string, Autotiles3D_Tile> _nameMap = new();
        ...
        {
            if (_map.TryGetValue(id, out var tile)) return tile;
            return null;
        }
        ...
        {
            if (_nameMap.TryGetValue(name, out var tile)) return tile;
        ...
        {
            foreach (var tile in Tiles) tile.SetGroupName(name);
        ...
        public void ConstructMapping()
        {
            ...
            for (var i = 0; i < Tiles.Count; i++)
            {
                ...
                if (!_nameMap.ContainsKey(Tiles[i].Name))
                    ...
                    Debug.LogWarning(
                        $"Autotiles3D:  You have multiple tiles with the same name (duplicated name: {Tiles[i].Name}). This is no longer permitted since Autotiles 1.3.\n Make sure to change your tiles to unique names");
            }
        }
    }
}