    ...
    public class TraderConfirmPopup : MonoBehaviour
    {
        ...
        private void UpdateUI()
        {
            ...
            if (!string.IsNullOrEmpty(key))
            {
                ...
                if (itemDescriptionText)
                    itemDescriptionText.text = Localizer.GetText(descTable, key + "_description", null);
            }
        ...
        private void OnConfirmClick()
        {
            ...
            if (_itemData is RelicInfo relic)
            {
                ...
                if (_isSellMode)
                {
                    if (relics.Contains(relic.key)) relics.Remove(relic.key);
                }
                else
                {
                    if (!relics.Contains(relic.key)) relics.Add(relic.key);
                }
            }
            else if (_itemData is PieceInfo piece)
            {
                ...
                if (_isSellMode)
                {
                    if (units.Contains(piece.key)) units.Remove(piece.key);
                }
                else
                {
                    if (!units.Contains(piece.key)) units.Add(piece.key);
                }
            }
            else if (_itemData is AugmentationInfo aug)
            {
                ...
                if (_isSellMode)
                {
                    if (augs.Contains(aug.Name.ToString())) augs.Remove(aug.Name.ToString());
                }
                else
                {
                    if (!augs.Contains(aug.Name.ToString())) augs.Add(aug.Name.ToString());
                }
            }
