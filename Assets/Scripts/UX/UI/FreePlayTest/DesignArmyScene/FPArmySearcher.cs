...
namespace UX.UI.FreePlayTest.DesignArmyScene
{
    ...
    public class FPArmySearcher : ArmySearcher
    {
        ...
        private new void FilterByCondition()
        {
            ...
            foreach (var pieceInfo in FPArmyDesign.Ins.board.Troops.Select(tr =>
                         AssetManager.Ins.PieceData[tr.PieceType]))
            {
                ...
                if (pieceInfo.rank == PieceRank.Champion && counts[pieceInfo] == 2) greyOutPieces.Add(pieceInfo);
                //Elite: Tối đa 4 quân giống nhau / biến thể của nha
                if (pieceInfo.rank == PieceRank.Elite && counts[pieceInfo] == 4) greyOutPieces.Add(pieceInfo);
                //Common: tối đa 10 quân giống nhau / biến thể của nhau
                if (pieceInfo.rank == PieceRank.Common && counts[pieceInfo] == 10) greyOutPieces.Add(pieceInfo);
