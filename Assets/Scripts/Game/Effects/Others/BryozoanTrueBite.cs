...
{
    public class BryozoanTrueBite : Effect, IStartTurnTrigger
    {
        private const int BuffRange = 2;
        private const int BuffDuration = 3;
        
        public BryozoanTrueBite(PieceLogic piece) : base(-1, -1, piece, "effect_bryozoan_true_bite")
        {
        ...
        public void OnCallStart(Action.Action lastMainAction)
        {
            ...
            foreach (var (rankOff, fileOff) in MoveEnumerators.AroundUntil(rank, file, BuffRange))
            {
                ...
                if (pOn == null || pOn == Piece || pOn.Color != Piece.Color) continue;
                ActionManager.EnqueueAction(new ApplyEffect(new TrueBite(BuffDuration, pOn), Piece));
            }
        }
    }
}