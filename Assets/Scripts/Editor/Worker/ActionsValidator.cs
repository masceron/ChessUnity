    ...
    public class ActionsValidator : IPreprocessBuildWithReport
    {
        ...
        private static bool ValidateFieldTypes(Type actionType)
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (field.GetCustomAttribute<MemoryPack.MemoryPackIgnoreAttribute>() != null) continue;
                
                if (IsTypeNetworkSafe(field.FieldType, out var failureReason)) continue;
                
                LogViolation($"Action '{actionType.Name}' contains a forbidden type. Trace: {field.Name} -> {failureReason}");
                clean = false;
            }
            return clean;
        }
