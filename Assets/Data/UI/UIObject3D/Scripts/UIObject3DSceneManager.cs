...
#if UNITY_EDITOR
#if UNITY_2017_2_OR_NEWER
        private static void InEditorCleanup(PlayModeStateChange stateChange)
        {
            InEditorCleanup();
        }
#endif

        private static void InEditorCleanup()
        {
            if (!EditorApplication.isPlayingOrWillChangePlaymode) return;
            var components = FindObjectsByType<UIObject3DScene>(FindObjectsSortMode.None);
