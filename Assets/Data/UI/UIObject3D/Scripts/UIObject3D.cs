...
namespace Data.UI.UIObject3D.Scripts
{
    ...
    public class UIObject3D : MonoBehaviour
    {
        ...
        {
            get => new(_TargetOffsetX, _TargetOffsetY);
            set
            {
        ...
        {
            get => _OverrideCalculatedTargetScale;
            set
            {
        ...
        {
            get => _CalculatedTargetScaleOverride;
            set
            {
        ...
        private void ClearObjectLayerFromCameras()
        {
            ...
            var otherCameras = FindObjectsByType<Camera>(FindObjectsSortMode.None);
            foreach (var c in otherCameras)
            {
                // don't modify the culling mask for other UIObject3DCameras
                if (c.GetComponent<UIObject3DCamera>()) continue;
        ...
        private void ClearObjectLayerFromLights()
        {
            ...
            var otherLights = FindObjectsByType<Light>(FindObjectsSortMode.None);
            foreach (var l in otherLights)
            {
                ...
                if (l.GetComponent<UIObject3DCamera>()) continue;
        ...
        void InEditorCleanup()
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode && !EditorApplication.isPlaying)
            {
                ...
                UIObject3DTimer.AtEndOfFrame(Cleanup, this, true);
            }
        }
#endif
        ...
        */
        ...
        private UIObject3DImage imageComponent
        {
            get
            {
                ...
                if (!_imageComponent)
                {
                    _imageComponent = gameObject.AddComponent<UIObject3DImage>();
                    setProperties = true;
                }
        ...
        internal Transform targetContainer
        {
            get
            {
                if (_targetContainer) return _targetContainer;
        ...
        private Transform target
        {
            get
            {
                if (!_target && started) SetupTarget();
        ...
        private void SetupTarget()
        {
            if (!_target)
            {
                ...
                _target = Instantiate(ObjectPrefab);
            }
        ...
        private void UpdateTargetPositioningAndScale()
        {
            ...
            var initial = targetBounds == default;

            if (initial)
            {
                // if our "Prefab" has no children, it is almost certainly a model
                var prefabIsModel = ObjectPrefab.gameObject.scene.name != "Null"; // ObjectPrefab.childCount == 0;
        ...
        private void SetLayerRecursively(Transform _transform, int layer)
        {
        ...
        private Camera targetCamera
        {
            get
            {
                if (!_targetCamera) SetupTargetCamera();
        ...
        protected Light cameraLight
        {
            get
            {
                if (!_cameraLight) SetupCameraLight();
        ...
        private void SetupCameraLight()
        {
            ...
            if (!_cameraLight) _cameraLight = targetCamera.gameObject.AddComponent<Light>();


            _cameraLight.enabled = EnableCameraLight;

            if (!EnableCameraLight) return;
        ...
        internal static int objectLayer
        {
            get
            {
                if (_objectLayer != -1) return _objectLayer;
