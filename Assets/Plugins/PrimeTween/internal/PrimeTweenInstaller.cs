...
using Window = UnityEditor.PackageManager.UI.Window;

[assembly: InternalsVisibleTo("PrimeTween.Internal")]
...
namespace PrimeTween
{
    internal class PrimeTweenInstaller : ScriptableObject
    {
        ...
        private void ResetReviewRequest()
        {
            ReviewRequest.ResetReviewRequest();
        }

        [ContextMenu(nameof(DebugReviewRequest))]
        private void DebugReviewRequest()
        {
            ReviewRequest.DebugReviewRequest();
        }
    }
    ...
    internal class InstallerInspector : Editor
    {
        ...
        internal const string newTgzPath =
            "Assets/Plugins/PrimeTween/internal/com.kyrylokuzyk.primetween-" + version + ".tgz";

        private const string documentationUrl = "https://github.com/KyryloKuzyk/PrimeTween";

        internal const string version = "1.3.8";
        private GUIStyle boldButtonStyle;
        private bool hasNewTgz;
        ...
        private static bool CheckPluginInstalled()
        {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (uninstallButtonStyle == null)
                uninstallButtonStyle = new GUIStyle(GUI.skin.button)
                    { normal = { textColor = installer.uninstallButtonColor } };
            if (wordWrapLabelStyle == null)
                wordWrapLabelStyle = new GUIStyle(GUI.skin.label)
                    { wordWrap = true, richText = true, margin = new RectOffset(4, 4, 8, 8) };
            ...
            if (!isInstalled)
            {
                if (Button("Install " + pluginName)) installPlugin();
                return;
            }

            if (hasNewTgz)
            {
                if (Button($"Update to {version}", boldButtonStyle)) ReviewRequest.OnPackageUpdate();
                Space(8);
            }

            if (Button("Documentation", boldButtonStyle)) Application.OpenURL(documentationUrl);
            ...
            var rpAsset = GraphicsSettings.
                ...
                renderPipelineAsset;
#endif
            var isUrp = rpAsset != null && rpAsset.GetType().Name.Contains("Universal");
            ...
            {
                EditorUtility.DisplayDialog(pluginName,
                    $"Please select the '{pluginName}' package in 'Package Manager', then press the 'Samples/Import' button at the bottom of the plugin's description.",
                    "Ok");
                Window.Open(pluginPackageId);
            }
#endif
            if (Button("Support")) Application.OpenURL("https://github.com/KyryloKuzyk/PrimeTween#support");
            ...
            if (Button("Uninstall", uninstallButtonStyle))
            {
                ...
                isInstalled = false;
                var msg =
                    $"Please remove the folder manually to uninstall {pluginName} completely: 'Assets/Plugins/{pluginName}'";
            ...
            if (EditorPrefs.GetBool(InsertCallbackBug.showInsertCallbackBugUi, false))
            {
                ...
                Label(
                    "The behaviour of 'Sequence.ChainCallback()' and 'InsertCallback()' was fixed in PrimeTween 1.2.0 so the code written with older versions may work differently in some cases.",
                    wordWrapLabelStyle);
                if (Button("Find potential issues")) InsertCallbackBug.Find();
                BeginHorizontal();
                if (Button("More info")) Application.OpenURL(InsertCallbackBug.moreInfoUrl);
                if (Button("Download version 1.1.22"))
                    Application.OpenURL(
                        "https://github.com/KyryloKuzyk/PrimeTween/blob/545dcc52769d52841e282c772e98c8984bfeb243/Benchmarks/Packages/com.kyrylokuzyk.primetween.tgz");
                EndHorizontal();
            }
        ...
        {
            if (File.Exists(newTgzPath)) MoveAndRenameTgzArchive();
            ...
            if (addRequest.Status == StatusCode.Success)
                ...
                Debug.LogError(
                    $"Please re-import the plugin from the Asset Store and check that the file exists: [{path}].\n\n{addRequest.Error?.message}\n");
        }
        ...
        private static void RevertTgzMeta()
        {
...
");
        ...
        private static void InitOnLoad()
        {
            AssetDatabase.importPackageCompleted += name =>
            {
                if (name.Contains(pluginName))
                {
                    ...
                    {
                        var installer =
                            AssetDatabase.LoadAssetAtPath<PrimeTweenInstaller>(
                                "Assets/Plugins/PrimeTween/PrimeTweenInstaller.asset");
                        EditorUtility
                            .FocusProjectWindow(); // this is important to show the installer object in the Project window
    ...
    internal static class FixedUpdateParameterMigration
    {
        internal static string[] FindLocalScriptGuids()
        {
            ...
            var folders = listRequest.Result
        ...
        internal static bool Process(string[] scripts, bool? fixAutomatically = null)
        {
            ...
            foreach (var guid in scripts)
            {
                ...
                if (!Regex.IsMatch(text, @"using PrimeTween\s*;")) continue;
                var parameterMatches = Regex.Matches(text, @"useFixedUpdate\s*:");
                if (parameterMatches.Count > 0)
                {
                    if (!fixAutomatically.HasValue) return true;
                    logSb.Clear();
                    if (fixAutomatically.Value)
                    {
                        fileSb.Clear();
                        fileSb.Append(text);
                        for (var i = parameterMatches.Count - 1; i >= 0; i--)
                        {
                        ...
                        logSb.Append(
                            $"PrimeTween automatically renamed ({parameterMatches.Count}) occurrences of 'useFixedUpdate' to 'updateType' in file '{textAsset.name}.cs':\n");
                    }
                    else
                    {
                        logSb.Append(
                            $"PrimeTween: please rename ({parameterMatches.Count}) occurrences of 'useFixedUpdate' to 'updateType' in file '{textAsset.name}.cs':\n");
                    }
    ...
    {
        private const string version = InstallerInspector.version;
        private const string canAskKey = "PrimeTween.canAskForReview";
        private const string versionKey = "PrimeTween.version";

        private static bool UNITY_2018
        {
            get
            {
#if UNITY_2018
        ...
        internal static void OnPackageUpdate()
        {
            ...
            {
                Debug.LogError(
                    $"The installation archive is missing: '{InstallerInspector.newTgzPath}'. Please re-import PrimeTween from Asset Store.");
                return;
            }

            var shouldAskForReview = true;
            ...
            if (FixedUpdateParameterMigration.Process(scriptGuids))
            {
                ...
                const string msg =
                    "'bool useFixedUpdate' parameter was changed to 'UpdateType updateType' in version 1.3.0, which will cause breaking changes in your current project.\n" +
                ...
                var response = EditorUtility.DisplayDialogComplex($"{InstallerInspector.pluginName} {version}",
                    $"The {msg}",
                    "Fix automatically",
                    "Cancel",
                    "Fix manually");
                var cancelMessage =
                    $"PrimeTween: update to {version} was cancelled. You can trigger update manually from 'Assets/Plugins/PrimeTween/PrimeTweenInstaller'.";
                if (response == 1)
                {
                ...
                if (!EditorUtility.DisplayDialog($"{InstallerInspector.pluginName} {version}",
                        "Please back up your project before proceeding.", "OK", "Cancel"))
                {
                    Debug.LogWarning(cancelMessage);
                    return;
                }

                var fixAutomatically = response == 0;
            ...
            if (UNITY_2018)
            {
                ...
                var path = $"file:../{InstallerInspector.tgzPath}";
                Client.Add(path);
            }
            else
            {
                EditorApplication
                    .ExecuteMenuItem("Assets/Refresh"); // AssetDatabase.Refresh() refreshes the project only partially
            }

            var prevVersion = savedVersion;
            ...
            if (shouldAskForReview) TryAskForReview();
        }

        private static void TryAskForReview()
        {
            ...
            var response = EditorUtility.DisplayDialogComplex("Enjoying PrimeTween?",
                ...
                "Never ask again",
                "");
            if (response == 0) OpenReviewsURL();
        }

        internal static void OnBeforeInstall()
        {
            log($"OnBeforeInstall {version}");
            if (string.IsNullOrEmpty(savedVersion)) savedVersion = version;
        }

        internal static void DisableReviewRequest()
        {
            EditorPrefs.SetBool(canAskKey, false);
        }

        internal static void OpenReviewsURL()
        {
            Application.OpenURL("https://assetstore.unity.com/packages/slug/252960#reviews");
        }
    ...
    internal static class InsertCallbackBug
    {
        ...
        private static Dictionary<short, OpCode> OpCodeDict;
        private static MethodInfo[] methodsWithBug;
        private static MethodInfo[] groupMethods;
        ...
        internal static void Find()
        {
            ...
            methodsWithBug = typeof(Sequence).GetMethods(BindingFlags.Public | BindingFlags.Instance)
                .Where(methodInfo => methodInfo.Name == nameof(Sequence.ChainCallback) ||
                                     methodInfo.Name == nameof(Sequence.InsertCallback))
            ...
            var methodAssemblyName = methodsWithBug[0].Module.Assembly.FullName;
            const BindingFlags findAll = BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.NonPublic |
                                         BindingFlags.Instance | BindingFlags.Static;
            var numPotentialIssues = AppDomain.CurrentDomain.GetAssemblies()
                .Where(assembly => assembly.GetReferencedAssemblies()
                    .Any(dependency => dependency.FullName == methodAssemblyName))
            ...
            if (numPotentialIssues == 0)
                Debug.Log(
                    $"PrimeTween updated to version {InstallerInspector.version}: no potential issues found in ChainCallback() and InsertCallback() usages.\n" +
                    $"More info: {moreInfoUrl}\n");

            var updateResponse = EditorUtility.DisplayDialogComplex(
                $"{InstallerInspector.pluginName} {InstallerInspector.version}",
                ...
                "Close",
                "");
            if (updateResponse == 0) Application.OpenURL(moreInfoUrl);
        }

        /// https://stackoverflow.com/a/33034906/1951038
        private static bool FindInMethod(MethodBase method)
        {
            ...
            using (var br = new BinaryReader(new MemoryStream(il)))
            {
                while (br.BaseStream.Position < br.BaseStream.Length)
                {
                    ...
                    var opCodeValue = firstByte == 0xFE
                        ...
                        : firstByte;
                    var opCode = OpCodeDict[opCodeValue];
                        ...
                        case OperandType.InlineMethod:
                            ...
                            if (method.Module.ResolveMethod(token) is MethodInfo resolvedMethod)
                            {
                                if (bugFound)
                                {
                                    ...
                                    {
                                        Debug.LogError(
                                            $"PrimeTween updated to version {InstallerInspector.version}: potential breaking change found in the '{method.DeclaringType}.{method.Name}()' method.\n" +
        ...
        private static bool isMethodWithBug(MethodInfo method)
        {
            foreach (var methodWithBug in methodsWithBug)
                if (methodWithBug.IsGenericMethodDefinition && method.IsGenericMethod)
                {
                    if (methodWithBug == method.GetGenericMethodDefinition()) return true;
                }
